...
const plugin = {
  ...
  constants: {
    ...
    tokenReplacePrompts: {
      ...
      thesaurus: "Provide 10 alternate word or phrases, in a single line, each word or phrase separated by ~ (example 'this~that'), for the following:"
    },
  },
  ...
  replaceText: {
    ...
    "thesaurus": async function(app, text) {
      ...
      const optionString = await this._callOpenAI(app, "thesaurus", messages);
      const optionList = optionString?.split("~");
      if (optionList?.length) {
        const selectedValue = await app.prompt("Choose a replacement", {
          inputs: [ { type: "select", label: "Synonyms", options: optionList.map(option => ({ label: option, value: option })) } ]
        });
