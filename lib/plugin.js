...
const plugin = {
  ...
  constants: {
    ...
    tokenReplacePrompts: {
      ...
      thesaurus: "Return a numbered list of the 10 best alternate word or phrases to replace <token> in the following document:"
    },
  },
    ...
    "thesaurus": async function(app, text) {
      ...
      if (optionList?.length) {
        const selectedValue = await app.prompt(`Choose a replacement for "${ text }"`, {
          inputs: [ { type: "select", label: `${ optionList.length } synonym${ optionList.length === 1 ? "" : "s" } found`, options: optionList.map(option => ({ label: option, value: option })) } ]
        });
